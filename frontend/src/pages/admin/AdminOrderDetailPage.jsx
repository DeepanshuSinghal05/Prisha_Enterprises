...
const AdminOrderDetailPage = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-medium ${badge.color}`}
      >
        <Icon className="mr-2" />
              ...
              {order.payment?.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-medium ${
                          order.payment[0]?.status === "captured"
                            ? "text-green-600"
                            : "text-yellow-600"
                        }`}
                      >
                        {order.payment[0]?.status
                          ? order.payment[0].status.charAt(0).toUpperCase() +
                            order.payment[0].status.slice(1)
                          : "N/A"}
                      </span>
                    </div>

                    {order.payment[0]?.gateway_payment_id && (
                    ...
                    {order.payment[0]?.gateway_order_id && (
                      <div className="pt-3 border-t border-gray-100">
                        <p className="text-gray-500 mb-1">Gateway Order ID</p>
                        <p className="font-mono text-xs text-gray-900 break-all">
                          {order.payment[0].gateway_order_id}
                        </p>
                      </div>
                    ...
                    {order.payment[0]?.created_at && (
                      <div className="pt-3 border-t border-gray-100">
                        <p className="text-gray-500 mb-1">Payment Date</p>
                        <p className="text-gray-900">
                          {formatDate(order.payment[0].created_at)}
                        </p>
                      </div>
                    )}
                  </div>
